  ...
  const saveProject = () => {
    ...
    if (isGuest) {
      ...
      return Promise.reject({
        success: false,
        msg: "Failed to save guest project",
      });
    }
  ...
  return (
    <>
      ...
      <div className="flex flex-col bg-white dark:bg-gray-900 min-h-screen pt-16">
        ...
        {/* Editor and Output */}
        <div
          className={`flex flex-1 overflow-hidden ${isFullScreen ? "fixed inset-0 z-50 pt-16" : ""}`}
        >
          {/* Editor Panel */}
          <div
            className={`
          ...
          >
            ...
            <div className="flex-1 overflow-auto">
              {output && output.length > 0 ? (
                <div
                  className={`p-4 font-mono text-sm ${error ? "bg-red-50/50 dark:bg-red-900/10" : "bg-gray-50/50 dark:bg-gray-800/50"}`}
                >
                  <pre
                    className={`${outputConfig.wrap ? "whitespace-pre-wrap" : "whitespace-pre"}`}
                  >
                    <table className="w-full border-collapse">
                      <tbody>
                        {Array.isArray(output) ? (
                          output.map(({ line, content, type }) => (
                            <tr
                              key={line}
                            ...
                            >
                              {outputConfig.showLineNumbers && (
                                <td className="select-none pr-4 text-gray-400 text-right align-top font-mono">
                                  {line}
                                </td>
                              )}
                              <td
                          ...
                          <tr>
                            {outputConfig.showLineNumbers && (
                              <td className="select-none pr-4 text-gray-400 text-right">
                                1
                              </td>
                            )}
                            <td className="break-all text-gray-800 dark:text-gray-200">
                              {output}
                            </td>
                          </tr>
                        )}
